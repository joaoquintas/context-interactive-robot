...
class BodyDetector(Detector):
	...
	works*.
	"""
	def __init__(self, cascade_fn="/Users/Utilizador/opencv-3.0.0/data/haarcascades/haarcascade_fullbody.xml", scaleFactor=1.2, minNeighbors=5, minSize=(30,30)):
		if not os.path.exists(cascade_fn):
			raise IOError("No valid cascade found for path=%s." % cascade_fn)
		self.cascade = cv2.CascadeClassifier(cascade_fn)
		self.scaleFactor = scaleFactor
...
if __name__ == "__main__":
	...
	for imgIdx, imgName, imgOut in zip(y,Names,X):
		...
		for bb in range(0,len(bounding_boxes)):
			cv2.rectangle(imgBGR, (int(bounding_boxes[bb][0]),int(bounding_boxes[bb][1])),(int(bounding_boxes[bb][2]),int(bounding_boxes[bb][3])),(255,0,0),1)
			cv2.putText(imgBGR,'annotation',(int(bounding_boxes[bb][0]),int(bounding_boxes[bb][1])),cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255,0,0), 1)
		...
		hit_haar = 0; error_haar = 0
		hit_hog = 0; error_hog = 0
		...
		for i,r in enumerate(detector.detect(imgOut)):
			...
			for bb in range(0,len(bounding_boxes)):
				dist_haar = np.sqrt(np.square(int(bounding_boxes[bb][0])-x0)+np.square(int(bounding_boxes[bb][1])-y0))+\
							np.sqrt(np.square(int(bounding_boxes[bb][2])-x1)+np.square(int(bounding_boxes[bb][3])-y1))
				# print("dist_haar: {}".format(dist_haar))
				if dist_haar < dist_threshold:
					hit_haar = hit_haar+1
				# else:
					# error_haar = error_haar+1
		...
		# calculate miss rates (false negative)
		# miss = num_persons - hit
		miss_haar = int(n_persons) - hit_haar




		# HOG detection of people in the image
		t_hog = timeit.timeit(stmt='hog.detectMultiScale(imgOut, winStride=(4, 4), padding=(8, 8), scale=1.2)', setup='from __main__ import hog, imgOut', number=1)
		...
		for (x, y, w, h) in rects:
			...
			for bb in range(0,len(bounding_boxes)):
				dist_hog = np.sqrt(np.square(int(bounding_boxes[bb][0])-x)+np.square(int(bounding_boxes[bb][1])-y))+ \
						   np.sqrt(np.square(int(bounding_boxes[bb][2])-(x+w))+np.square(int(bounding_boxes[bb][3])-(y+h)))
				# print("dist_hog: {}".format(dist_hog))
				if dist_hog < dist_threshold:
					hit_hog = hit_hog+1
				# else:
				# 	error_hog = error_hog+1
		...
		# calculate miss rates (false negative)
		# miss = num_persons - hit
		miss_hog = int(n_persons) - hit_hog
		...
		# print("\n{},{},{},{},{},{},{},{},{},{},{},{},{}".format(imgIdx,imgName,n_persons,t_haar,n_det_haar,hit_haar,error_haar,miss_haar,t_hog,n_det_hog,hit_hog,error_hog,miss_hog))

		# write row to file
		# prepare file to write results to file
		# with open('some.csv', 'a', newline='') as f:
		# 	writer = csv.writer(f)
		# 	row = [imgIdx,imgName,n_persons,t_haar,n_det_haar,hit_haar,error_haar,miss_haar,t_hog,n_det_hog,hit_hog,error_hog,miss_hog]
		# 	writer.writerow(row)
		# f.close()
